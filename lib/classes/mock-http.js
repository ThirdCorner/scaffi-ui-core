...
function _filter(url, obj) {
    ...
    var parseFilters = function parseFilters(parseFilter) {
        var foundFilter = false;
        var filters = {};
        _lodash2.default.each(parseFilter, function (filter, name) {
            if (_lodash2.default.isObject(filter)) {
                var returnedFilters = parseFilters(filter);
                if (returnedFilters && setFilter(filters, name, returnedFilters)) {
                    foundFilter = true;
                }
            } else {
                if (_lodash2.default.isString(filter) && filter.length == 0) {
                    filter = null;
                }

                if (filter && filter != null && (filter.length > 0 || _lodash2.default.isNumber(filter))) {
    ...
    var shouldFilterRecord = function shouldFilterRecord(record, filters) {
        ...
        _lodash2.default.each(filters, function (name, value) {
    ...
    if (params.filter && _lodash2.default.isObject(params.filter)) {
        ...
        obj = _lodash2.default.find(obj, function (record) {
